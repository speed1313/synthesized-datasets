...
def main(dtype: str, tp: int):
    ...
    root_dir = Path("datasets/wiki_paraphrase/gemma2_27b")
    batch_size = 10000
    ...
    dataset: ds.Dataset = ds.load_dataset("hpprc/jawiki-paraphrases", "collection", split="train", num_proc=16)

    counter_path = root_dir / "counter.json"
    if counter_path.exists():
        with counter_path.open() as f:
            counter = json.load(f)
    else:
        generated: ds.Dataset = ds.load_dataset("hpprc/jawiki-paraphrases", "generated", split="train", num_proc=16)
        counter = {idx: 0 for idx in range(len(dataset))}
        for row in tqdm(generated):
            counter[row["id"]] += 1
        with counter_path.open("w") as f:
            json.dump(counter, f)

    indices = [int(idx) for idx, count in counter.items() if count <= 2]
